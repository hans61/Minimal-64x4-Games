...
#org 0x8000
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Start Game ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
begin:      LDI 0xfe SDB 0xffff ; SP initialisieren
            JPS initVariables
            JPS loadHighscore
            JPS _Clear
            JPS printHighscore
...
checkKey1:  LDB oldPlayerXPosLo
            ...
            BCC checkKey2
            LDI 0x07
checkKey2:  SDB playerFrameNumber
            JPS delayLong
            JPA redrawPlayer
deathSound:
...
checkLevelComplete:
            ...
            LDI 0x00
            SDB allGoldCollected
            SDB cntLevelComplete
loopLevelComplete:
...
;jsr OSWORD                          ;SOUND
            ...
            LDI 0x00
            SDB playingSoundAllGold
            SDB soundIndexAllGold
checkHoleDigging:
            LDB oldPlayerXPosLo
            SDB tmpXPosLo
            LDB oldPlayerXPosHi
...
grUpdatePos:
            ...
            BNE grUp1
            LDI 0x00
grUp1:      SDB playerXBlkInternalDecimalOffset
            CPI 0x00
            BNE grUp2
            INB playerXBlockPos
grUp2:      LDI 0x01
            SDB playerDirection ;i.e. player is facing right
            LDB playerXBlkInternalDecimalOffset
            CPI 0x00
            BEQ grInspectNewPos
...
glUpdatePos:
            ...
            BNE glUp1
            LDI 0x06
glUp1:      SDB playerXBlkInternalDecimalOffset ;internal block offset-=20
            CPI 0x06
            BNE glUp2
            DEB playerXBlockPos
glUp2:      LDI 0x00
...
gdUpdatePos:
            ...
            SDB tmpXPosHi
            LDI 0x00
            SDB playerXBlkInternalDecimalOffset
            LDI 0x02
            AD.B tmpYPos
            INB playerYBlkInternalDecimalOffset
            LDB playerYBlkInternalDecimalOffset
            ...
            BNE gdUp1
            LDI 0x00
            SDB playerYBlkInternalDecimalOffset
...
checkHole:
            JPS KeyHandler
            LDI <holeArray
            SDB holeFillCounters+0
            LDI >holeArray
            SDB holeFillCounters+1
            LDB holeIndex
            SDZ 1
            LL1
            ADZ 1
...
tryDiggingL:
            LDI 0x00
tryDigging:
            SDB playerDirection ; 0 = left 1 = right
            CLB _digLeft
            CLB _digRight
            LDB playerDirection
            CPI 0x00
...
startDigging:
            ...
            SDB playerXBlkInternalDecimalOffset
            LDI 0x05
            SDB indexAnimDigHole ;first frame for digging a hole
            LDI 0x01
            SDB diggingHole   ;busy digging a hole now
...
saveHolePos:
            LDZ 0
            SDB xBlockPosDigHole
            SDB mapX
            LDB playerYBlockPos
            INC
            SDB yBlockPosDigHole
            SDB mapY
            JPS calcMapPtr2Pixel
            LDB xpos16+0
            SDB xPosDigHoleLo
...
setHoleInfo:
            ...
            SDR ptr1          ; holeFillCounters = 0x82
            INW ptr1          ; Pointer to holeXBlockPos
            LDB xBlockPosDigHole
            SDR ptr1          ; holeXBlockPos = xBlockPosDigHole
            INW ptr1          ; Pointer to holeYBlockPos
            LDB yBlockPosDigHole
            SDR ptr1          ; holeYBlockPos = yBlockPosDigHole
            RTS
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
redrawPlayer:
            ...
            CPI 0x01          ;brick
            BNE rpDone
            SDB playerIsDead  ;permanently trapped in a hole, i.e. dead
...
coll1:      0x0000
            ...
            LDI 0x05
            SDB soundIndexGold ;start sound for collecting a gold ingot
...
replaceEscapeLadder:
            ...
            BNE nextTileInRow
            LDI 0x03          ;ladder
            SDR ptr1          ;replace escape ladder with regular ladder in tilemap
            SDB spriteId
            JPS calcMapPtr2Pixel
...
updateEnemy:
            ...
            SDB tmpXPosLo
            LDR enemyXPosHi
            SDB tmpXPosHi     ;xpos 16-bit in pixels not blocks
            LDR enemyYPos
            SDB tmpYPos       ;ypos in pixels not blocks
            LDR enemyFrames
            SDB enemyFrameNumber
            LDR enemyFalling
            CPI 0x00
            BEQ ueNotFalling
; enemy is falling, update vertical position
            ...
            INC
            SDR enemyYBlkInternalDecimalOffset ;internal block offset
            CPI 0x07
            BNE updE01
            LDI 0x00
            SDR enemyYBlkInternalDecimalOffset
            LDR enemyYBlockPos
            INC
            SDR enemyYBlockPos ;update block pos if needed
...
ueStopFalling:
            LDI 0x00
            SDR enemyFalling  ;mark as no longer falling
...
; reached bottom of hole and possibly leave gold ingot behind just above it
            ...
            LDI 0x05
            SDR ptrM          ;STA ($80),y ;place gold ingot in tilemap directly above enemy
            SDB spriteId
            LDI 0x00
            SDR enemyHoldsGoldIngot ;no longer holding gold ingot
            JPS KeyHandler
            JPS calcMapPtr2Pixel ; Input mapX, mapY (Block) Output: xpos16, ypos8 = 14 * mapX, 14 * mapY
...
ueEnemyReachedBottomOfHole:
            ...
            SDR enemyInHoleCountdown ;counts the time spent in a hole
            MIZ 0x75,0
            MIZ 0x00,1
...
ueSelectOtherUpDownFrame:
            LDI 0x27
ueSetUpDownFrame:
            SDB enemyFrameNumber ;enemy up/down sprite #1 or #2
            LDI 2
...
; enemy trapped in hole, make it respawn at top of screen
            LDI 0x00
            SDR enemyXBlkInternalDecimalOffset
            SDR enemyYBlkInternalDecimalOffset
            SDR enemyInHoleHeight
            SDR enemyInHoleCountdown
            SDR enemyYBlockPos
...
uePrepareToRespawnEnemy:
            ...
            SDR enemyXBlockPos
            LDI 0x0a
            SDR enemyRespawnCountdown ;wait a short while before actually respawning enemy
            LDI 0x1f
            SDB enemyFrameNumber ;empty frame
            MIZ 0x75,0
            MIZ 0x00,1
...
; setup respawn at countdown==9
            ...
            SDB tmpXPosHi
            LDB ypos8
            SDB tmpYPos       ;calc enemy pixel xpos and ypos at respawn point
            LDI 0x2f
            SDB enemyFrameNumber ;enemy respawn frame #1
            JPA ueCountdownRespawn
ueNextRespawnFrame:
            CPI 0x06
            BNE ueFinalRespawnFrame
            LDI 0x30
            SDB enemyFrameNumber ;enemy respawn frame #2
            JPA ueCountdownRespawn
ueFinalRespawnFrame:
            CPI 0x03
            BNE ueCountdownRespawn
            LDI 0x28
            SDB enemyFrameNumber ;enemy up/down #3 (falling)
ueCountdownRespawn:
            LDR enemyRespawnCountdown
            DEC
            SDR enemyRespawnCountdown
            JPA ueCheckSpecialCases
ueDoneClimbingOutOrRespawning:
            JPS KeyHandler
            LDI 0x00
            SDB distanceToPassageGoingLeft
            SDB distanceToPassageGoingRight
...
ueMovingLeftAndDownImpossible:
            LDI 0xff          ;lda #$ff
falseLabel1:
            SDB distanceToPassageGoingLeft ;$ff means there is no route going left // &&&& falseLabel1 generated because of self-modifying code referencing &1900
ueStartScanningRowsRightOfEnemy:
            LDR enemyXBlockPos ;ldy enemyXBlockPos,x    -> A = X-Position Fänger
            SDZ 1             ; Y = A = X-Position Fänger
...
ueMovingRightAndDownImpossible:
            LDI 0xff          ;lda #$ff
            SDB distanceToPassageGoingRight ;$ff means there is no route going right
...
ueEnemyStartsFalling:
            LDI 0x01
            SDR enemyFalling  ;sta enemyFalling,x                  ;no solid floor tile, so enemy starts falling
            LDI 0x28
            SDB enemyFrameNumber ;sta enemyFrameNumber                ;select enemy up/down frame #3 (falling)
            JPA ueCheckSpecialCases
...
ueSetEnemyUpDownFrame:
            SDB enemyFrameNumber ;switch between enemy up/down frames
            ...
            LDI 0x00
ueSetEnemy1:
            SDR enemyYBlkInternalDecimalOffset ;sta enemyYBlkInternalDecimalOffset,x
            CPI 0x00
            BNE ueCheckSpecialCases
            LDR enemyYBlockPos ;lda enemyYBlockPos,x
            INC
            SDR enemyYBlockPos ;sta enemyYBlockPos,x
            JPA ueCheckSpecialCases
...
ueMovingLeftAndUpImpossible:
            LDI 0xff
            SDB distanceToPassageGoingLeft ;$ff means there is no route going left
...
ueEnemyMovingUpLadder:
            ...
            SDB tmpXPosHi
            LDI 0x00          ;lda #$00
            SDR enemyXBlkInternalDecimalOffset ;sta enemyXBlkInternalDecimalOffset,x ;align enemy horizontally (snap to ladder &&&& could be improved)
            LDR enemyFrames   ;lda enemyFrames,x
            CPI 0x26
...
uePlayerIsDead:
            LDI 0x01
            SDB playerIsDead  ;enemy occupies same block pos as player so that kills the player instantly
            JPA ueCheckSpecialCases
...
ueSetHangingOnLineFrameB:
            ...
            ADI 0x29          ;adc #$29
            SDB enemyFrameNumber ;sta enemyFrameNumber ;select correct frame for hanging on a line
...
ueMoveRightUpdatePos:
            ...
            SDR enemyXBlockPos ;sta enemyXBlockPos,x
            LDI 0x00
ueMoveR1:    SDR enemyXBlkInternalDecimalOffset ;sta enemyXBlkInternalDecimalOffset,x
            JPA ueCheckSpecialCases
...
ueSetHangingOnLineFrameB2:
            ...
            LAB enemyFrameOffset ;lda enemyFrameOffset,y
            ADI 0x2c          ;adc #$29
            SDB enemyFrameNumber ;select correct frame for hanging on a line
...
ueSetWalkingLeftFrameB:
            ...
            LAB enemyFrameOffset ;lda enemyFrameOffset,y
            ADI 0x23          ;adc #$29
            SDB enemyFrameNumber ;select correct frame for walking left
ueMoveLeftUpdatePos:
            LDI 0x02
            ...
            SDR enemyXBlockPos ;sta enemyXBlockPos,x
            LDI 0x06
ueMoveLe1:  SDR enemyXBlkInternalDecimalOffset ;sta enemyXBlkInternalDecimalOffset,x
...
; hole is not occupied so enemy is about to fall into this hole
            ...
            SDR enemyInHoleHeight ;sta enemyInHoleHeight,x ;at very top of hole
            JPA ueEnemyFallingInHole ;jump always ;bne ueEnemyFallingInHole
ueEnemyStartFalling:
            LDI 0x01
            SDR enemyFalling  ;sta enemyFalling,x
ueEnemyFallingInHole:
            LDI 0x28
            SDB enemyFrameNumber ;set falling enemy frame
            JPA ueRedrawEnemy
...
; enemy has gold ingot and is vertically aligned to 10px block
            ...
            LDZ 1
            SDB tmp02         ;remember current tile offset
            ADI 0x1c          ;adc #$1c
...
; enemy can leave the gold ingot on an empty tile directly above a ladder
            LDI 0x00          ;lda #$00
            SDR enemyHoldsGoldIngot ;sta enemyHoldsGoldIngot,x
...
uePossiblyPickupGoldIngot:
            ...
            BNE ueRedrawEnemy
            LDI 0x01          ;lda #$01
            SDR enemyHoldsGoldIngot ;sta enemyHoldsGoldIngot,x ;enemy now holds this gold ingot
            LDI 0x00          ;lda #$00
...
bpEn:       PHS
            LDI 0x00          ;lda #$00
            SDR tileMapPtr    ;sta ($74),y    ;erase this enemy tile, because there are too many
...
printHighscore:
            ...
            JPS drawLineScore
            LDI 9 SDB _YPos
            LDI <Highscore SDB ptrM+0
            LDI >Highscore SDB ptrM+1
            MIZ 1,0
printHSloop:
            LDI 4 SDB _XPos
...
printHS2:   LDI '.' PHS JPS printCharXY PLS
            LDI 3 ADW _XPos
            LDI 11 SDB tmp05  ; name 11 characters
...
printHS4:   PHS INB _XPos JPS printThreeDigitNumber PLS
            LDI 2 ADW _XPos
            INW ptrM
            LDB ptrM+0 SDB ptrScore+0
            LDB ptrM+1 SDB ptrScore+1
            JPS printScoreXY
            LDI 4
printHS5:   ADW ptrM
            INB _YPos
...
cmpHighscore:
            ...
            LDI 10 SDB counterHGR ; 10 entries in the list
nxtHGR1:    MIZ 4,0           ; 4 bytes bcd for score
            LDB ptrM+0 SDB ptr1+0
            LDB ptrM+1 SDB ptr1+1 ; ptr1 -> high byte Liste[index]
            LDI <score+3 SDB ptrE+0
            LDI >score+3 SDB ptrE+1 ; ptrE -> current score high byte
nxtHGR2:    LDR ptr1          ; score byte
            SDB tmp01         ; tmp01 = score byte
            LDR ptrE          ; liste byte
...
nxtHGR3:    DEB counterHGR    ; Enter score in list
            ...
            LDB counterHGR LL4 SDB counterHGR ; Counter for moving the list 16 bytes per entry
            LDI 15 SUW ptrM   ; to the first byte of the entry in the list
            LDI <Highscore+160 SDB ptr1+0 SDB ptrE+0
            LDI >Highscore+160 SDB ptr1+1 SDB ptrE+1 ; ptr1 = ptrE -> letztes byte der Liste
            LDI 16 SUW ptr1   ; copy from SDB ptr1 to SDB ptrE
...
nxtHGR3d:   SDR ptrM         ; valid character
            PHS
            INW ptrM
            JPS printCharXY PLS
            DEZ 0
...
nxtHGRcr:   LDZ 0 DEZ 0
            BCC nxtHGRcr1
            LDI ' ' SDR ptrM INW ptrM
            JPA nxtHGRcr
nxtHGRcr1:  LDB level
            SDR ptrM INW ptrM
            LDI <score SDB ptrE+0
            LDI >score SDB ptrE+1
            MIZ 4,0
nxtHGR5:    LDR ptrE SDR ptrM
            INW ptrE INW ptrM
            DEZ 0
            BNE nxtHGR5
            JPS _Clear
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
loadHighscore:
            LDI <fileNameHGS SDB _ReadPtr+0
            LDI >fileNameHGS SDB _ReadPtr+1
            JPS _FindFile
            CPI 0
            BNE hgrFound1
...
hgrFound1:  AIV 24,PtrA JPS OS_FlashA ; file found, skip header
            MIZ 160,0
            MIV Highscore,PtrB
hgrFound2:  RDR PtrA        ; copy block from A -> to B
            SDR PtrB        ; store in RAM
...
key_entry:
            ...
            BEQ is_pressed
key_rts:    RTS
is_left:    LDB pressed SDB _left ORI 1 SDB pressed CLB _right RTS
is_right:   LDB pressed SDB _right ORI 1 SDB pressed CLB _left RTS
is_up:      LDB pressed SDB _up ORI 1 SDB pressed CLB _down RTS
is_down:    LDB pressed SDB _down ORI 1 SDB pressed CLB _up RTS
is_digLeft: LDB pressed SDB _digLeft ORI 1 SDB pressed CLB _digRight RTS
is_digRight: LDB pressed SDB _digRight ORI 1 SDB pressed CLB _digLeft RTS
is_escape:  LDB pressed SDB _escape ORI 1 SDB pressed RTS
is_quit:    LDB pressed SDB _quit ORI 1 SDB pressed RTS
is_pressed: LDB pressed SDB _pressed ORI 1 SDB pressed RTS
...
printChar:      LDS 4       ; Y
                LL6 SDB vAddr+0 ;
                LDS 4         ; Y
                RL7 ANI 63 ADI >ViewPort SDB vAddr+1
                LDS 5         ; Xhight
                DEC
                LDS 6         ; Xlow
                RL6 ANI 63 ADI 12 OR.B vAddr+0
                LDS 6 ANI 7 SDB shift ; Xlow
                LDI 10 SDB lineCnt ; scnt loop counter 8 byte + 2 byte space
                LDS 3         ; char
                CPI 0x60
...
loadEnemyPointer:
            ...
            SDB enemyXBlockPos+0
            LDI >enemyArray   ; high enemy array
            SDB enemyXBlockPos+1
            LDB enemyIndex    ; 0..5
            LL1               ; enemy index =*2
            ADW enemyXBlockPos
            LDR enemyXBlockPos
            ...
            LDR enemyXBlockPos
            SDB enemyXBlockPos+1
            SDB enemyYBlockPos+1
            SDB enemyXBlkInternalDecimalOffset+1
            SDB enemyYBlkInternalDecimalOffset+1
            SDB enemyXPosLo+1
            SDB enemyXPosHi+1
            SDB enemyYPos+1
            SDB enemyFrames+1
            SDB enemyFalling+1
            SDB enemyCounter+1
            SDB enemyInHoleHeight+1
            SDB enemyInHoleCountdown+1
            SDB enemyRespawnCountdown+1
            SDB enemyHoldsGoldIngot+1
            PLS
            SDB enemyXBlockPos+0
            SDB enemyYBlockPos+0
            SDB enemyXBlkInternalDecimalOffset+0
            SDB enemyYBlkInternalDecimalOffset+0
            SDB enemyXPosLo+0
            SDB enemyXPosHi+0
            SDB enemyYPos+0
            SDB enemyFrames+0
            SDB enemyFalling+0
            SDB enemyCounter+0
            SDB enemyInHoleHeight+0
            SDB enemyInHoleCountdown+0
            SDB enemyRespawnCountdown+0
            SDB enemyHoldsGoldIngot+0
            INW enemyYBlockPos
            LDI 2
...
m001:       AIV 22,PtrA JPS OS_FlashA                       ; search for target addr
            RDR PtrA SDZ PtrB+0 INV PtrA JPS OS_FlashA      ; destination addr -> PtrC, PtrD
            RDR PtrA SDZ PtrB+1 INV PtrA JPS OS_FlashA
            MVV PtrA,PtrC MZZ PtrA+2,PtrC+2                 ; PtrA -> PtrC
                              ; hier noch gr��e pr�fen PtrB
...
m003:       MIZ 224,0
            MIV levelData,PtrB
m004:       RDR PtrA        ; copy block from A -> to B
            SDR PtrB        ; store in RAM
            INW PtrA INW PtrB JPS OS_FlashA
            DEZ 0
            BNE m004
